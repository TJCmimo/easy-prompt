...
- 材料缺失：保留 `<粘贴代码/报错/日志/截图/素材>` 一类可替换占位符。
...
- 把已确认的文件路径、函数名、报错关键原文、日志位置、构建/测试命令、已做的修改写入成品对应字段（材料、bug 现象与失败观察入 `材料`，命令入 `处理方法` 或 `验收`）。这些写入不算虚构；反之，明明在上文且通过下述删除测试却只留占位符才是失职。
...
3. `材料`：放用户提供或会话中已确认的任务材料；缺失时只放占位符。
